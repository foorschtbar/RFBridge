...
// Constants - HW pins
const int HWPIN_TRANSMITTER = D1;
...
void HTMLHeader(const char *section, unsigned int refresh, const char *url)
{
  ...
  snprintf(title, 50, "RFBridge@%s - %s", hostname, section);
...
void handleSend()
{
  ...
  else
  {
    ...
    html += "<input type='input' name='data' >";
...
void MQTTprocessCommand(JsonObject &json)
{
  ...
  {
    String hexstr = json["hex"].as<String>();
    hexstr.toCharArray(buffer, sizeof(buffer));
    uint32_t hexbuffer;
    toHex(buffer, &hexbuffer);
    sendRF(hexbuffer);
  }
  else if (json.containsKey("dec"))
  {
    unsigned long data = json["dec"].as<unsigned long>();
    sendRF(data);
  }
}
...
void loadDefaults()
{
  ...
  memcpy(cfg.mqtt_prefix, "rfbridge", sizeof(cfg.mqtt_prefix) / sizeof(*cfg.mqtt_prefix));
}
...
void setup(void)
{
  ...
  // Enable SwitchRF
  mySwitch.enableTransmit(HWPIN_TRANSMITTER);
